...
fn main() -> Result<(), Box<dyn error::Error>> {
    let default_faucet_pubkey = solomka_cli_config::Config::default().keypair_path;
...
mod tests {
    ...
    fn test_append_primordial_accounts_to_genesis() {
        ...
        genesis_accounts.insert(
            solomka_sdk::pubkey::new_rand().to_string(),
            Base64Account {
                owner: solomka_sdk::pubkey::new_rand().to_string(),
                balance: 2,
                executable: false,
