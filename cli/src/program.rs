...
impl ProgramSubCommands for App<'_, '_> {
    fn program_subcommands(self) -> Self {
        ...
        .subcommand(
            SubCommand::with_name("deploy")
                .about("Deploy a non-upgradeable program. Use `solomka program deploy` instead to deploy upgradeable programs")
...
fn report_ephemeral_mnemonic(words: usize, mnemonic: bip39::Mnemonic) {
    ...
    eprintln!(
        "`solomka-keygen recover` and the following {}-word seed phrase:",
        words
    );
    eprintln!("{}\n{}\n{}", divider, phrase, divider);
    eprintln!("To resume a deploy, pass the recovered keypair as the");
    eprintln!("[BUFFER_SIGNER] to `solomka program deploy` or `solomka program write-buffer'.");
    eprintln!("Or to recover the account's lamports, pass it as the");
    eprintln!(
        "[BUFFER_ACCOUNT_ADDRESS] argument to `solomka program close`.\n{}",
        divider
    );
}
