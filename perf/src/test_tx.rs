    ...
    R: CryptoRng + RngCore,
{
    ...
    slots.sort_unstable();
    slots.dedup();
    let switch_proof_hash = rng.gen_bool(0.5).then(|| solomka_sdk::hash::new_rand(rng));
    vote_transaction::new_vote_transaction(
        slots,
        solomka_sdk::hash::new_rand(rng), // bank_hash
        solomka_sdk::hash::new_rand(rng), // blockhash
