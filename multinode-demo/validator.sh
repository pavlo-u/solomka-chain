...
wallet() {
  (
    set -x
    $solomka_cli --keypair "$identity" --url "$rpc_url" "$@"
  )
}
...
[[ -r "$identity" ]] || $solomka_keygen new --no-passphrase -so "$identity"
[[ -r "$vote_account" ]] || $solomka_keygen new --no-passphrase -so "$vote_account"
[[ -r "$authorized_withdrawer" ]] || $solomka_keygen new --no-passphrase -so "$authorized_withdrawer"
