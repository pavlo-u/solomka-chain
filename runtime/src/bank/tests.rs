...
fn test_race_register_tick_freeze() {
    ...
    let p = solomka_sdk::pubkey::new_rand();
...
fn test_rent_exempt_executable_account() {
    ...
    let mut account = AccountSharedData::new(account_balance, 0, &solomka_sdk::pubkey::new_rand());
...
fn test_rent_eager_collect_rent_in_partition() {
    ...
    for feature_id in FeatureSet::default().inactive {
        if feature_id != solomka_sdk::feature_set::set_exempt_rent_epoch_max::id() {
...
fn do_test_bank_update_rewards_determinism() -> u64 {
    ...
    let stake_id1 = solomka_sdk::pubkey::new_rand();
    let stake_account1 = crate::stakes::tests::create_stake_account(123, &vote_id, &stake_id1);
    let stake_id2 = solomka_sdk::pubkey::new_rand();
...
fn test_bank_load_program() {
    ...
    let programdata_key = solomka_sdk::pubkey::new_rand();
        ...
        ) + std::marker::Send,
{
    ...
    // Set up initial bank
    let mut genesis_config =
        create_genesis_config_with_leader(10, &solomka_sdk::pubkey::new_rand(), 374_999_998_287_840)
            .genesis_config;
...
fn test_call_precomiled_program() {
    ...
    let instruction =
        solomka_sdk::secp256k1_instruction::new_secp256k1_instruction(&secp_privkey, message_arr);
    let tx = Transaction::new_signed_with_payer(
        &[instruction],
        Some(&mint_keypair.pubkey()),
        &[&mint_keypair],
...
fn test_cap_accounts_data_allocations_per_transaction() {
    ...
    assert_eq!(
        ...
        Err(TransactionError::InstructionError(
            ...
            solomka_sdk::instruction::InstructionError::MaxAccountsDataAllocationsExceeded,
        )),
    );
}
...
fn test_stake_account_consistency_with_rent_epoch_max_feature(
    ...
    let expected_initial_rent_epoch = if rent_epoch_max_enabled_initially {
        bank.activate_feature(&solomka_sdk::feature_set::set_exempt_rent_epoch_max::id());
