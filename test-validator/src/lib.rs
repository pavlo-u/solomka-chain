...
use {
    ...
    solomka_program_runtime::compute_budget::ComputeBudget,
...
impl TestValidatorGenesis {
    ...
    pub fn add_accounts_from_json_files(&mut self, accounts: &[AccountInfo]) -> &mut Self {
        for account in accounts {
            let account_path =
                solomka_program_test::find_file(account.filename).unwrap_or_else(|| {
    ...
    pub fn add_account_with_file_data(
        ...
        self.add_account(
            ...
            AccountSharedData::from(Account {
                lamports,
                data: solomka_program_test::read_file(
                    solomka_program_test::find_file(filename).unwrap_or_else(|| {
    ...
    pub fn add_program(&mut self, program_name: &str, program_id: Pubkey) -> &mut Self {
        let program_path = solomka_program_test::find_file(&format!("{}.so", program_name))
            .unwrap_or_else(|| panic!("Unable to locate program {}", program_name));

        self.programs.push(ProgramInfo {
            program_id,
            loader: solomka_sdk::bpf_loader::id(),
            program_path,
        });
        self
    }
...
impl TestValidator {
    ...
    fn initialize_ledger(
        ...
        for (address, account) in solomka_program_test::programs::spl_programs(&config.rent) {
            accounts.entry(address).or_insert(account);
        }
        for program in &config.programs {
            let data = solomka_program_test::read_file(&program.program_path);
            accounts.insert(
                program.program_id,
                AccountSharedData::from(Account {
        ...
        let mut genesis_config = create_genesis_config_with_leader_ex(
            ...
            config.rent,
            solomka_sdk::genesis_config::ClusterType::Development,
