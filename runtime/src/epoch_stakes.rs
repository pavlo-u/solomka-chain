...
pub(crate) mod tests {
    use {
        super::*, crate::vote_account::VoteAccount, solomka_sdk::account::AccountSharedData,
    ...
    fn test_parse_epoch_vote_accounts() {
        ...
        let vote_accounts_map: HashMap<Pubkey, Vec<VoteAccountInfo>> = (0..10)
            .map(|_| {
                let node_id = solomka_sdk::pubkey::new_rand();
                (
                    node_id,
                    iter::repeat_with(|| {
                        let authorized_voter = solomka_sdk::pubkey::new_rand();
                        VoteAccountInfo {
                            vote_account: solomka_sdk::pubkey::new_rand(),
