...
impl ProgramSubCommands for App<'_, '_> {
    fn program_subcommands(self) -> Self {
        ...
        .subcommand(
            SubCommand::with_name("deploy")
                .about("Deploy has been removed. Use `sonoma program deploy` instead to deploy upgradeable programs")
...
fn report_ephemeral_mnemonic(words: usize, mnemonic: bip39::Mnemonic) {
    ...
    eprintln!("`sonoma-keygen recover` and the following {words}-word seed phrase:");
    eprintln!("{divider}\n{phrase}\n{divider}");
    ...
    eprintln!("[BUFFER_ACCOUNT_ADDRESS] argument to `sonoma program close`.\n{divider}");
}
