...
while [[ $timeout -gt 0 ]]; do
  set +e
  output=$($solomka_cli "${args[@]}" transaction-count --commitment finalized)
  rc=$?
  set -e
...
(
  set -x
  $solomka_cli "${args[@]}" address
  $solomka_cli "${args[@]}" balance
  $solomka_cli "${args[@]}" ping --count 5 --interval 0
  $solomka_cli "${args[@]}" balance
)
